	...
	lists:flatten(
	  lists:sublist(
	    lists:map(fun({AppVsn, _ErtsVsn, AppPath}) -> 
			      lists:flatten([" <a href=\"", renderable_path(AppPath, DocRoot),"/index.html\">", AppVsn, "</a> |"]) 
		      end, 
...
renderable_path(Path, DocRoot) ->
    ?INFO_MSG("create renderable path from ~p and ~p~n", [Path, DocRoot]),
